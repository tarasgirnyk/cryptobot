...
if not defined CRYPTOBOT_PY (
  echo [X] Python 3.11+ не знайдено. Встанови Python і повтори.
  pause
  exit /b 1
)
...
rem --- Ensure ccxt is available ---
%CRYPTOBOT_PY% -c "import ccxt" 1>nul 2>nul
if errorlevel 1 (
  echo [i] Встановлюю ccxt...
  %CRYPTOBOT_PY% -m pip install "ccxt>=4.4,<5"
)

echo.
%CRYPTOBOT_PY% check_connect.py
echo.
pause
